...
exports.cancelOffboarding = exports.getOffboardingStatus = exports.processScheduledOffboardings = exports.initiateUserOffboarding = exports.OffboardingType = exports.OffboardingStatus = void 0;
...
(function (OffboardingStatus) {
    OffboardingStatus["ACTIVE"] = "active";
    OffboardingStatus["PENDING_DEACTIVATION"] = "pending_deactivation";
    OffboardingStatus["DEACTIVATED"] = "deactivated";
    OffboardingStatus["ARCHIVED"] = "archived";
})(OffboardingStatus = exports.OffboardingStatus || (exports.OffboardingStatus = {}));
/**
 * Типы деактивации
 */
var OffboardingType;
(function (OffboardingType) {
    OffboardingType["VOLUNTARY_RESIGNATION"] = "voluntary_resignation";
    OffboardingType["INVOLUNTARY_TERMINATION"] = "involuntary_termination";
    OffboardingType["CONTRACT_EXPIRATION"] = "contract_expiration";
    OffboardingType["ADMINISTRATIVE"] = "administrative";
})(OffboardingType = exports.OffboardingType || (exports.OffboardingType = {}));
/**
 * Инициация процесса деактивации пользователя
 */
exports.initiateUserOffboarding = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    const { auth: authContext, data } = request;
    // Проверяем права администратора или HR
    if (!authContext || !authContext.token.admin) {
        throw new https_1.HttpsError('permission-denied', 'Only administrators can initiate offboarding');
    }
    const offboardingRequest = Object.assign(Object.assign({}, data), { effectiveDate: new Date(data.effectiveDate), requestedBy: authContext.uid, dataRetentionDays: data.dataRetentionDays || 90 });
    try {
        // Создаем запрос на деактивацию
        const offboardingId = `offboard_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        await db.collection('offboarding_requests').doc(offboardingId).set(Object.assign(Object.assign({}, offboardingRequest), { offboardingId, status: OffboardingStatus.PENDING_DEACTIVATION, createdAt: firestore_1.Timestamp.now(), effectiveDate: firestore_1.Timestamp.fromDate(offboardingRequest.effectiveDate), steps: {
        ...
        throw new https_1.HttpsError('internal', 'Failed to initiate offboarding process');
    }
});
/**
 * Выполнение процесса деактивации пользователя
 */
async function executeOffboarding(offboardingId) {
    ...
    try {
        ...
        await db.collection('offboarding_requests').doc(offboardingId).update({
            ...
            lastAttempt: firestore_1.Timestamp.now()
        });
        throw error;
    }
}
...
async function terminateAllUserSessions(userId) {
    ...
    await db.collection('user_sessions').doc(userId).update({
        allSessionsTerminatedAt: firestore_1.Timestamp.now(),
 ...
 * Архивация данных пользователя
 */
async function archiveUserData(userId, retentionDays) {
    ...
    await db.collection('user_archives').doc(archiveId).set({
        userId,
        archivedAt: firestore_1.Timestamp.now(),
        retentionUntil: firestore_1.Timestamp.fromDate(new Date(Date.now() + retentionDays * 24 * 60 * 60 * 1000)),
 ...
 */
async function notifyTeamMembers(userId, offboardingType) {
 ...
 */
async function notifyHRDepartment(userId, offboarding) {
 ...
 */
exports.processScheduledOffboardings = (0, scheduler_1.onSchedule)({
 ...
 */
exports.getOffboardingStatus = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    const { auth: authContext, data } = request;
    if (!authContext) {
    ...
    const offboardingDoc = await db.collection('offboarding_requests').doc(offboardingId).get();
    if (!offboardingDoc.exists) {
        throw new https_1.HttpsError('not-found', 'Offboarding request not found');
    }
 ...
 */
exports.cancelOffboarding = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    const { auth: authContext, data } = request;
    if (!authContext || !authContext.token.admin) {
        throw new https_1.HttpsError('permission-denied', 'Only administrators can cancel offboarding');
    }
    ...
    const offboarding = offboardingDoc.data();
    if ((offboarding === null || offboarding === void 0 ? void 0 : offboarding.status) !== OffboardingStatus.PENDING_DEACTIVATION) {
        throw new https_1.HttpsError('failed-precondition', 'Can only cancel pending offboarding requests');
    }
    await db.collection('offboarding_requests').doc(offboardingId).update({
        status: 'CANCELLED',
...
//# sourceMappingURL=offboardingFunctions.js.map