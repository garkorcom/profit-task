...
exports.recalculateCOGS = exports.onTimeEntryStatusChange = void 0;
...
__exportStar(require("./rbacFunctions"), exports);
// Импортируем функции деактивации пользователей
__exportStar(require("./offboardingFunctions"), exports);
// Импортируем функции MFA
__exportStar(require("./mfaFunctions"), exports);
// Импортируем функции стартуемости проектов
__exportStar(require("./startability"), exports);
// Storage proxy functions
__exportStar(require("./storage-proxy"), exports);
/**
 ...
 */
exports.onTimeEntryStatusChange = functions.firestore
    ...
    const shouldCalculateCOGS = before.status !== after.status &&
 ...
 */
async function calculateAndStoreCOGS(userId, entryId, timeEntry) {
 ...
 */
async function getLaborRateForDate(ownerId, userId, date) {
    const db = admin.firestore();
    ...
    for (const query of queries) {
        ...
        if (!snapshot.empty) {
            ...
            if (!rate.effectiveTo || rate.effectiveTo >= date) {
                return Object.assign({ id: rateDoc.id }, rate);
            }
        }
    }
    return null;
 ...
 */
function createCostSnapshot(timeEntry, laborRate) {
    return {
 ...
 */
function getAppliedMultiplier(timeEntry, laborRate) {
 ...
 */
function calculateTotalCost(timeEntry, laborRate) {
...
function createCOGSRecord(params) {
    ...
    return {
        ...
        amount: totalCost,
        currency: 'RUB',
        costType: includeMode === 'COGS' ? 'direct_labor' : 'burdened_labor',
        // Временные данные
        hours: timeEntry.hours,
        hourlyRate: totalCost / timeEntry.hours,
        // Связи
        timeEntryId: entryId,
        costSnapshotId,
 ...
 */
exports.recalculateCOGS = functions.https.onCall(async (data, context) => {
 ...
 */
async function deleteExistingCOGSRecords(userId, timeEntryId) {
    var _a;
    const db = admin.firestore();
    const batch = db.batch();
    // Удаляем COGSRecord
    ...
    if (timeEntry.exists && ((_a = timeEntry.data()) === null || _a === void 0 ? void 0 : _a.costSnapshotId)) {
