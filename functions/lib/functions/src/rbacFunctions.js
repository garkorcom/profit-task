...
exports.getRBACStats = exports.evaluateConditionalPermissions = exports.checkUserPermission = exports.createDelegation = exports.createUserGroup = exports.assignRoleToUser = exports.createDynamicRole = void 0;
 ...
 */
exports.createDynamicRole = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    if (!hasRoleManagementRights(actorClaims)) {
        ...
        throw new functions.https.HttpsError('permission-denied', 'Only owners and managers can create roles');
    }
    try {
        ...
        if (!validPermissions) {
            throw new functions.https.HttpsError('invalid-argument', 'Invalid permissions provided');
        }
        // Создаем роль
...
exports.assignRoleToUser = functions.https.onCall(async (data, context) => {
    ...
    if (!hasUserManagementRights(actorClaims)) {
        throw new functions.https.HttpsError('permission-denied', 'Only owners and managers can assign roles');
    }
    try {
        ...
        if (!conditionsValid) {
            throw new functions.https.HttpsError('failed-precondition', 'User does not meet role conditions');
        }
        if (temporary) {
 ...
 */
exports.createUserGroup = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated');
    }
    const actorId = context.auth.uid;
    ...
    if (!hasGroupManagementRights(actorClaims)) {
        throw new functions.https.HttpsError('permission-denied', 'Only owners and managers can create user groups');
    }
    try {
 ...
 */
exports.createDelegation = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated');
    }
    ...
    try {
        ...
        if (!canDelegate) {
            throw new functions.https.HttpsError('permission-denied', 'Cannot delegate permissions you do not have');
        }
 ...
 */
exports.checkUserPermission = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated');
    }
    try {
 ...
 */
exports.evaluateConditionalPermissions = functions.https.onCall(async (data, context) => {
    var _a, _b;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated');
    }
    try {
        const { roleId, evaluationContext, permission, permissionContext } = data;
        ...
        const conditionResults = await Promise.all(role.conditions.map(condition => evaluateConditionOnServer(condition, evaluationContext)));
        ...
        await createAuditLog({
            ...
            actorId: ((_b = context.auth) === null || _b === void 0 ? void 0 : _b.uid) || 'system',
            action: 'CONDITIONAL_PERMISSION_CHECK',
            resource: 'conditional_permissions',
 ...
 */
async function evaluateConditionOnServer(condition, context) {
 ...
 */
function evaluateTimeConditionOnServer(condition, context) {
 ...
 */
function evaluateLocationConditionOnServer(condition, context) {
    if (!context.location) {
        return {
            passed: false,
 ...
 */
async function evaluateProjectConditionOnServer(condition, context) {
    var _a;
    ...
    switch (condition.field) {
        case 'project_member': {
            ...
            try {
                ...
                const isMember = (_a = projectData === null || projectData === void 0 ? void 0 : projectData.members) === null || _a === void 0 ? void 0 : _a.includes(context.userId);
                return {
 ...
 */
async function evaluateDepartmentConditionOnServer(condition, context) {
    ...
    try {
        ...
        const userDepartment = userData === null || userData === void 0 ? void 0 : userData.department;
        switch (condition.field) {
            case 'department': {
 ...
 */
function evaluateCustomConditionOnServer(condition, context) {
    var _a;
    const customValue = (_a = context.customAttributes) === null || _a === void 0 ? void 0 : _a[condition.field];
 ...
 */
exports.getRBACStats = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated');
    }
    const actorClaims = context.auth.token;
    if (!hasAdminRights(actorClaims)) {
        throw new functions.https.HttpsError('permission-denied', 'Only administrators can view RBAC statistics');
    }
    try {
        ...
        const stats = {
            ...
            usersWithMultipleRoles: users.filter(u => { var _a; return (((_a = u.primaryRoles) === null || _a === void 0 ? void 0 : _a.length) || 0) > 1; }).length,
 ...
 */
function hasRoleManagementRights(claims) {
 ...
 */
function hasUserManagementRights(claims) {
 ...
 */
function hasGroupManagementRights(claims) {
    return claims.role === 'owner' || claims.role === 'manager' ||
 ...
 */
async function updateRoleHierarchy(parentRoleId, childRoleId, operation) {
    const parentRoleRef = db.collection('rbac_roles').doc(parentRoleId);
    const parentDoc = await parentRoleRef.get();
    if (parentDoc.exists) {
        const parentRole = parentDoc.data();
 ...
 */
async function validateRoleConditions(role, userId) {
    if (!role.conditions || role.conditions.length === 0) {
        return true;
    }
    ...
    for (const condition of role.conditions) {
        ...
        switch (condition.operator) {
            ...
            case 'not_in':
                ...
                break;
            case 'contains':
                if (!fieldValue || !fieldValue.toString().includes(condition.value))
                    return false;
                break;
 ...
 */
async function getUserRBACProfile(userId) {
 ...
 */
async function updateEffectivePermissions(userId, permissions) {
 ...
 */
async function canUserDelegatePermissions(userId, permissions) {
    const userProfile = await getUserRBACProfile(userId);
    ...
    return permissions.every(permission => userPermissions.globalPermissions.has(permission));
}
/**
 * Добавление делегирования в профиль
 */
async function addDelegationToProfile(userId, delegationId, type) {
    const userProfile = await getUserRBACProfile(userId);
    if (type === 'granted') {
        const delegation = await db.collection('authority_delegations').doc(delegationId).get();
        if (delegation.exists) {
            userProfile.grantedDelegations.push(delegation.data());
        }
    }
    else {
        const delegation = await db.collection('authority_delegations').doc(delegationId).get();
        if (delegation.exists) {
            userProfile.receivedDelegations.push(delegation.data());
        }
    }
    await updateUserRBACProfile(userId, userProfile);
}
 ...
 */
function shouldRecalculate(permissions) {
 ...
 */
function checkPermissionInSet(permissions, permission, context) {
    ...
    if (context && permissions.contextualPermissions.has(context)) {
        ...
        if (contextPerms === null || contextPerms === void 0 ? void 0 : contextPerms.has(permission)) {
            const sources = permissions.sources.filter(s => s.permission === permission);
            return {
                hasPermission: true,
                sources,
 ...
 */
function calculateMostUsedPermissions(roles) {
 ...
 */
async function calculateRoleDistribution(roles) {
    const distribution = [];
 ...
 */
async function countTemporaryRoleAssignments() {
...
//# sourceMappingURL=rbacFunctions.js.map