...
export const updateUserRole = functions.https.onCall(async (data, context) => {
  ...
  if (!hasAdminRights(actorClaims)) {
    ...
    throw new functions.https.HttpsError(
      'permission-denied', 
      'Only owners and managers can change user roles'
    );
  }
  ...
  try {
    ...
    await userRef.update({
      role: newRole,
      isActive: newRole !== 'pending_approval', // Активируем при назначении роли
      updatedAt: admin.firestore.FieldValue.serverTimestamp()
    });
 ...
 */
export const updateUserHourlyRate = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated');
  }
  ...
  if (!hasFinancialRights(actorClaims)) {
    ...
    throw new functions.https.HttpsError(
      'permission-denied', 
      'Only owners and accountants can change hourly rates'
    );
  }
  ...
  if (!targetUserId || typeof newHourlyRate !== 'number') {
    throw new functions.https.HttpsError(
      'invalid-argument', 
      'targetUserId and newHourlyRate are required'
    );
  }
 ...
 */
export const refreshUserCustomClaims = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be authenticated');
  }
  ...
  if (!hasAdminRights(actorClaims)) {
    throw new functions.https.HttpsError(
      'permission-denied', 
      'Only admins can refresh Custom Claims'
    );
  }
 ...
 */
function hasFinancialRights(claims: any): boolean {
 ...
 */
async function createAuditLog(entry: Partial<AuditLogEntry>): Promise<void> {
  try {
