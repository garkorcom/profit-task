 ...
 */
export const initiateMFASetup = onCall(
  { region: 'us-central1' },
  async (request) => {
    ...
    try {
      ...
      await db.collection('mfa_setup_temp').doc(userId).set({
        ...
        expiresAt: Timestamp.fromDate(new Date(Date.now() + 15 * 60 * 1000)) // 15 минут
      });
      ...
      throw new HttpsError('internal', 'Failed to initiate MFA setup');
    }
  }
);
 ...
 */
export const completeMFASetup = onCall(
  { region: 'us-central1' },
  async (request) => {
    ...
    if (!setupId || !verificationCode) {
      throw new HttpsError('invalid-argument', 'setupId and verificationCode are required');
    }
    ...
    try {
      ...
      if (setupId !== userId) {
        throw new HttpsError('permission-denied', 'Invalid setup ID');
      }
      ...
      if (!setupDoc.exists) {
        throw new HttpsError('not-found', 'MFA setup session not found or expired');
      }
      ...
      if (setup.expiresAt.toDate() < new Date()) {
        ...
        throw new HttpsError('deadline-exceeded', 'MFA setup session expired');
      }
      ...
      batch.set(mfaRef, {
        ...
        backupCodes: setup.backupCodes.map((code: string) => ({
      ...
      throw new HttpsError('internal', 'Failed to complete MFA setup');
    }
  }
);
...
export const verifyMFACode = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      if (!mfaDoc.exists || !mfaDoc.data()?.isEnabled) {
        throw new HttpsError('failed-precondition', 'MFA is not enabled for this user');
      }
      ...
      if (isBackupCode) {
        ...
        if (isValid && usedBackupCode !== null) {
          ...
          mfaData.backupCodes[usedBackupCode].usedAt = Timestamp.now();
 ...
 * Отключение MFA
 */
export const disableMFA = onCall(
  { region: 'us-central1' },
  async (request) => {
    const { auth: authContext, data } = request;
    ...
    try {
      ...
      if (customClaims.role === 'admin' || customClaims.role === 'owner') {
        throw new HttpsError('failed-precondition', 'MFA cannot be disabled for administrators');
      }
...
export const getMFAStatus = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      const mfaData = mfaDoc.data()!;
      const usedCodes = mfaData.backupCodes?.filter((code: any) => code.isUsed).length || 0;
      const totalCodes = mfaData.backupCodes?.length || 0;
      ...
      throw new HttpsError('internal', 'Failed to get MFA status');
    }
  }
);
 ...
 */
export const enableSMSBackup = onCall(
  { region: 'us-central1' },
  async (request) => {
    const { auth: authContext, data } = request;
    ...
    if (!phoneNumber || !confirmationCode) {
      throw new HttpsError('invalid-argument', 'Phone number and confirmation code are required');
    }
    ...
    try {
      ...
      if (!mfaDoc.exists || !mfaDoc.data()?.isEnabled) {
        throw new HttpsError('failed-precondition', 'MFA must be enabled before adding SMS backup');
      }
      ...
      throw new HttpsError('internal', 'Failed to enable SMS backup');
    }
  }
);
 ...
 */
export const sendSMSCode = onCall(
  { region: 'us-central1' },
  async (request) => {
    const { auth: authContext } = request;
    ...
    try {
      ...
      if (!mfaDoc.exists || !mfaDoc.data()?.methods.sms) {
        throw new HttpsError('failed-precondition', 'SMS backup is not enabled for this user');
      }
      ...
      await db.collection('sms_codes_temp').doc(userId).set({
        ...
        createdAt: Timestamp.now(),
        expiresAt: Timestamp.fromDate(new Date(Date.now() + 5 * 60 * 1000)) // 5 минут
      });
      ...
      throw new HttpsError('internal', 'Failed to send SMS code');
    }
  }
);