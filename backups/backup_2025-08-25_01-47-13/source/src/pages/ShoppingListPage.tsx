...
const makeKey = (projectId: string | undefined, text: string) => `${projectId || 'ALL'}::${normalizeItem(text).toLowerCase()}`;
...
const ShoppingListPage: React.FC = () => {
  ...
  const items = useMemo(() => {
    ...
    tasks.forEach(t => {
      ...
      const raw = (t.whatToBuy || '').split(/\r?\n|,|;|·|•/).map(s => normalizeItem(s)).filter(Boolean);
      raw.forEach(text => {
        const key = makeKey(projectFilter ? projectFilter : undefined, text);
        if (index.has(key)) {
          const i = index.get(key)!; list[i].count += 1;
        } else {
          index.set(key, list.length);
          list.push({ key, text, projectId: t.projectId, projectName: t.projectName, count: 1 });
        }
      });
    });
    // Фильтр по поиску
