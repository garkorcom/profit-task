...
const TimeTrackingDiagnostics: React.FC = () => {
  ...
  const runDiagnostics = async () => {
    ...
    if (tasks.length === 0) {
      ...
      if (inProgressTasks.length > 1) {
        results.push({
          ...
          details: inProgressTasks.map(t => t.name),
          action: async () => {
            setFixing(true);
    ...
    const toJsDate = (value: any): Date | null => {
      ...
      if (typeof value?.toDate === 'function') { try { const d = value.toDate(); return isNaN(d.getTime()) ? null : d; } catch { return null; } }
    ...
    const todayEntries = timesheet.filter(e => toISODate(e.date) === today);
    ...
    if (orphanedTasks.length > 0) {
      results.push({
        ...
        details: orphanedTasks.map(t => ({ id: t.id, name: t.name })),
        action: async () => {
          setFixing(true);
