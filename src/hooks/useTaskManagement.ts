/**
 * ============================================================================
 * USE TASK MANAGEMENT - ЦЕНТРАЛЬНЫЙ ХУК СИСТЕМЫ SSOT
 * ============================================================================
 * 
 * useTaskManagement представляет ядро унифицированной системы управления
 * задачами, объединяя в себе всю бизнес-логику работы с EstimateTask и
 * ProjectTask через единый интерфейс.
 * 
 * АРХИТЕКТУРНАЯ РОЛЬ:
 * ══════════════════════
 * 
 * Хук выполняет функцию фасада (Facade Pattern) для сложной системы
 * управления задачами, скрывая детали реализации и предоставляя простой
 * API для компонентов пользовательского интерфейса.
 * 
 * КЛЮЧЕВЫЕ ВОЗМОЖНОСТИ:
 * ════════════════════════
 * 
 * 1. УНИФИЦИРОВАННЫЕ CRUD ОПЕРАЦИИ
 *    ├─ createTask(): создание задач любого типа с автоматической валидацией
 *    ├─ updateTask(): обновление с проверкой бизнес-правил и прав доступа
 *    ├─ deleteTask(): удаление с проверкой зависимостей
 *    └─ bulkOperations(): массовые операции для оптимизации производительности
 * 
 * 2. REAL-TIME СИНХРОНИЗАЦИЯ
 *    ├─ Firebase Firestore subscriptions для мгновенных обновлений
 *    ├─ Optimistic updates для улучшения UX
 *    ├─ Automatic reconnection при сбоях сети
 ...
 *    └─ Selective updates для минимизации ре-рендеринга
 * 
 * 4. ПРОДВИНУТАЯ ФИЛЬТРАЦИЯ И СОРТИРОВКА
 *    ├─ Multi-criteria filtering с поддержкой сложных условий
 *    ├─ Dynamic sorting с кэшированием результатов
 *    ├─ Search capabilities с полнотекстовым поиском
 *    └─ Pagination для больших наборов данных
 * 
 * 5. ВАЛИДАЦИЯ И БИЗНЕС-ПРАВИЛА
 *    ├─ Schema validation через конфигурационные объекты
 *    ├─ Business rules enforcement (переходы статусов, разрешения)
 *    ├─ Cross-field validation для сложных зависимостей
 *    └─ Custom validation rules через plugin архитектуру
 * 
 * 6. СИСТЕМА РАЗРЕШЕНИЙ (RBAC)
 *    ├─ Role-based access control интеграция
 *    ├─ Field-level permissions для тонкой настройки доступа
 *    ├─ Dynamic permissions основанные на контексте задачи
 *    └─ Audit trail для отслеживания изменений
 * 
 * 7. ПРОИЗВОДИТЕЛЬНОСТЬ И ОПТИМИЗАЦИЯ
 *    ├─ Debounced operations для снижения нагрузки на сервер
 *    ├─ Memoization критических вычислений
 *    ├─ Lazy loading связанных данных
 *    └─ Request batching для минимизации сетевых вызовов
 * 
 * ПАТТЕРНЫ ПРОЕКТИРОВАНИЯ:
 * ═══════════════════════════
 * 
 * • FACADE - Единый интерфейс для сложной подсистемы
 * • OBSERVER - Real-time обновления через подписки
 * • STRATEGY - Переключаемые алгоритмы сортировки/фильтрации  
 * • COMMAND - Отменяемые операции с историей изменений
 * • FACTORY - Создание задач разных типов через единый интерфейс
 * 
 * ИНТЕГРАЦИЯ С ЭКОСИСТЕМОЙ:
 * ════════════════════════════
 * 
 * ┌─────────────────┐    ┌──────────────────┐    ┌─────────────────┐
 * │   UI Components │────│ useTaskMgmt Hook │────│   Firebase API  │
 * └─────────────────┘    └──────────────────┘    └─────────────────┘
 *           │                       │                       │
 *           ▼                       ▼                       ▼
 * ┌─────────────────┐    ┌──────────────────┐    ┌─────────────────┐
 * │ Form Validation │    │ Business Logic   │    │ Real-time Sync  │
 * └─────────────────┘    └──────────────────┘    └─────────────────┘
 *           │                       │                       │
 *           ▼                       ▼                       ▼
 * ┌─────────────────┐    ┌──────────────────┐    ┌─────────────────┐
 * │ RBAC System     │    │ Cache Management │    │ Offline Support │
 * └─────────────────┘    └──────────────────┘    └─────────────────┘
 * 
 ...
 * });
 * 
 * // Создание задачи с автоматической типизацией
 * const newEstimateTask = await createTask({
 ...
 * });
 * 
 * // Фильтрация с type-safe условиями
