...
import { PlayArrow as StartIcon, Stop as StopIcon, Timer as TimerIcon, Work as WorkIcon, PhotoCamera as CameraIcon } from '@mui/icons-material';
...
const TimeTrackingPage: React.FC = () => {
  ...
  const [endComment, setEndComment] = useState<string>('');
  ...
  const handleStartWork = async () => {
    ...
    if (!startPhoto) {
      alert('Добавьте фото ДО начала работ');
      return;
    }
  ...
  const handleStopWork = async () => {
    if (!endPhoto) {
      alert('Добавьте фото ПОСЛЕ завершения работ');
      return;
    }
    try {
        ...
        {isWorking && currentSession ? (
          ...
          >
            ...
            <CardActions sx={{ justifyContent: 'center', pb: 2 }}>
              ...
              <Button component="label" variant="outlined" startIcon={<CameraIcon />} sx={{ mr: 2 }}>
                {endPhoto ? 'Фото ПОСЛЕ выбрано' : 'Фото ПОСЛЕ'}
                <input type="file" accept="image/*" capture="environment" hidden onChange={(e) => setEndPhoto(e.target.files?.[0] || null)} />
              </Button>
              <Button
                variant="contained"
                color="error"
            ...
            {selectedProject && selectedTask && (
              <Alert severity="success" sx={{ mt: 3 }}>
                ...
                <Button component="label" variant="outlined" startIcon={<CameraIcon />} sx={{ mt: 2 }}>
                  {startPhoto ? `Фото ДО выбрано: ${startPhoto.name}` : 'Добавить фото ДО'}
