...
const EstimateConstructorPage: React.FC = () => {
  ...
  }, [currentUser, estimateId, versions.length]);
  ...
  const handleCreateVersion = async () => {
    ...
    try {
      const versionBase = (estimate.version || '1.0');
      const nextVersion = versionBase.split('.').map((v, i) => 
  ...
  const handleCreateShareLink = async () => {
    ...
    try {
      const link = await createShareLink(currentUser.uid, estimateId, {
        isPublic: true,
        allowComments: false,
        requireAuth: false
      });
              ...
              <CardContent>
                ...
                {estimate.taxRate && (
                  <Box display="flex" justifyContent="space-between" mb={1}>
                    ...
                    <Typography>{(((estimate.subtotal || 0) * (estimate.taxRate || 0) / 100)).toFixed(2)} {getCurrencySymbol(estimate.currency)}</Typography>
                  </Box>
                )}
                {estimate.discountRate && (
                  <Box display="flex" justifyContent="space-between" mb={1}>
                    <Typography>Скидка ({estimate.discountRate}%):</Typography>
                    <Typography>-{(((estimate.subtotal || 0) * (estimate.discountRate || 0) / 100)).toFixed(2)} {getCurrencySymbol(estimate.currency)}</Typography>
                  </Box>
                )}
                <Divider sx={{ my: 1 }} />
